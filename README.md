...
1. 环境变量 `JWT_SECRET`
2. YAML 配置中的 `jwt.secret`
3. 用户本地运行时配置文件 `config.local.yaml`
4. 若以上都不存在或不安全：首次启动自动生成随机密钥并写入用户目录
...
- `your_jwt_secret_key`、`your_jwt_secret_key_change_in_production` 等占位符会被视为不安全并拒绝使用。
- 测试环境（`NODE_ENV=test`）仍允许测试密钥，避免影响自动化测试。
